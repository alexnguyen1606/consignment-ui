            ...
            $(res).each(function (index, item) {
                ...
                if(item.checkHaveChilds==0){
                    if(item.status == 0){
                        ...
                        row += '<span   id="id_cate_root_tree' + item.id + '"  id_cate_root="' + item.id + '" onclick="showChildCateTestTree(this)" class="carett">&nbsp;<i title="Khóa" class="fas fa-bookmark text-warning"></i>&nbsp;' + item.nameTest + '</span>'
                    }
                }else{
                    if(item.status == 0){
                        ...
                        row += '<span   id="id_cate_root_tree' + item.id + '"  id_cate_root="' + item.id + '" onclick="showChildCateTestTree(this)" class="carett">&nbsp;<i title="Khóa" class="far fa-dot-circle text-warning"></i>&nbsp;' + item.nameTest + '</span>'
                    }
                }
                ...
                $(res).each(function (index, items) {
                    ...
                    if(items.level ==1){
                        if(items.status == 0){
                            ...
                            row += '<span    id_cate_root=' + items.id + '  id="id_cate_root' + items.id + '"   onclick="showChildCateTestTree(this)"  class="carett"><i title="Khóa" class="fas fa-home text-warning"></i>&nbsp;' + items.nameTest + '</span>'
                        }
                        ...
                        if(items.status == 0){
                            ...
                            row += '<span   id_cate_root=' + items.id + '  id="id_cate_root' + items.id + '"   onclick="OnOffActiveTree(this)" style="cursor: pointer"><i title="Khóa" class="far fa-dot-circle text-warning"></i></i>&nbsp;' + items.nameTest + '</span>'
                        }
                     }
